...
def p_exposed_growth(params, substep, state_history, prev_state):
    exposed_population = prev_state['daily_infection_rate']*prev_state['infected']*(prev_state['susceptible']/(prev_state['susceptible']+ prev_state['exposed'] + prev_state['infected'] + prev_state['recovered']))
...
def p_daily_infection_rate_growth(params, substep, state_history, prev_state):
    daily_infection_rate = prev_state['daily_infection_rate'] - (0.04 * params['recovering_rate'])
    return {'daily_infection_rate_growth': daily_infection_rate}
...
# 𝛽(t):  expected amount of people an infected person infects per day
def s_daily_infection_rate(params, substep, state_history, prev_state, policy_input):
    updated_daily_infection_rate = policy_input['daily_infection_rate_growth']
    return ('daily_infection_rate', max(updated_daily_infection_rate, 0))