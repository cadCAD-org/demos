...
if __name__ == "__main__":
    ...
    if(not args.key):
        ...
        try:
            df.drop(columns=['convert_rate'], inplace=True)
        except:
            pass
    ...
    df = df.set_index('block_date').join(rate_df.set_index('date')).reset_index(drop=True)
    df['convert_rate'] = df['convert_rate'].astype(float)
